...
export class EditEstablishmentComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    this.establishmentService.getOneEstablishment(id).toPromise().then(data => this.establishment = data);
  }

  update(establishment: Establishment) {
    this.establishmentService.putUpdateOneEstablishment(establishment).subscribe(() => this.location.back());
